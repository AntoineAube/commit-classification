...
rm -rf "$DOCKER_DATA"
mkdir -p "$DOCKER_DATA"
./create-sonarscanner-data.sh "$REPOS_DIR" docker/data $NUMBER_OF_WEEKS repos_to_scan 
#&> /dev/null
...
USER_TOKEN=6ded6e2c1a9d093d470fccf090a32256426124e1
docker/docker_run_sonar-scanner.sh $USER_TOKEN $NUMBER_OF_WEEKS "$REPOS_DIR"

./repos_checkout_latest.sh $REPOS_DIR